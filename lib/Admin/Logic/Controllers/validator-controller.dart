...
class ValidatorController extends GetxController {
  ...
  static validationFile(var column, var file) {
    if(file!=null) {
      if (column['validators'] != null) {
        for (int i = 0; i < column['validators'].length; i++) {
          if (column['validators'][i]['type'] == 'image') {
              print('ValidatorController.validationFile>>${file}>>${file.runtimeType}');
          ...
          if (column['validators'][i]['type'] == 'xlx') {
              if (file.extension!.toLowerCase() == 'xlx') {
                return [true, ''];
              } else {
                return [false, column['validators'][i]['message']];
              }

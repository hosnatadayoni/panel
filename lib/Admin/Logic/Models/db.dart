...
import '../Controllers/connection-controller.dart';
...
class DB {
  ...
  paginate() async {
    ...
    int countShowRow =
    int.parse(await MainController.getInfoTable('${this.tableName}')['schema']['countShowRow'].toString());
    int currentPage =
    int.parse(await MainController.getInfoTable('${this.tableName}')['schema']['currentPage'].toString());
  ...
  infoPage() async {
    ...
    await MainController.getInfoTable('${this.tableName}')['schema']['countShowRow'].toString());
    int perPage = countShowRow != null ? countShowRow : 10;
  ...
  getRecords() async {
    ...
    if (MainController.SubMenuList[index]['schema']['online'] == true ) {
      if(this.whereList.length==0 && this.orWhereList.length==0) {
        ...
        data.forEach((d) {
          boxList.forEach((b) async {
            if(d['_id']!=b['_id']){
              print('DB.getRecords dataaaa>>${d['_id']}>>>${b['_id']}');
              data.addAll(await getDataTypeOfField(boxList));
          }
          });
        });
      }
